# -*- coding: utf-8 -*-
"""
Created on Tue Apr 13 09:41:49 2021
...
#1st Question 
# read data
df = pd.read_csv("DailyDemandForecastingOrders.csv", sep=';')
df.head()
df.describe()
df.shape
...
#X = df[['Week','Day','Non-urgent order','Urgent order','Order type A','Order type B','Order type C','Fiscal sector orders','Traffic controller sector orders','Banking orders (1)','Banking orders (2)','Banking orders (3)']] 
...
for train_index, test_index in cv.split(X):
    ...
    result = scoreResults(model = rr_cv
                          ,X_train = X_train
                          ,X_test = X_test
                          ,y_train = y_train
                          ,y_test = y_test)
    
    y_train_pred = rr_cv.predict(X_train)
    y_test_pred = rr_cv.predict(X_test)
    print(f"{iter}. veri kesiti")
    print(f"Train R2 Score: {result[0]:.4f} MSE: {result[2]:.4f}")
    print(f"Test R2 Score: {result[1]:9.4f} MSE: {result[3]:.4f}\n")
    iter += 1
    scores.append(rr_cv.score(X_test, y_test))
...
df2 = pd.DataFrame(Week,columns=['Week'])
df2['Day'] = pd.Series(Day, index=df.index)
df2['Non-urgent order'] = pd.Series(df['Non-urgent order'], index=df.index)
df2['Order type A'] = pd.Series(df['Order type A'], index=df.index)
df2['Order type B'] = pd.Series(df['Order type B'], index=df.index)
df2['Banking orders (1)'] = pd.Series(df['Banking orders (1)'], index=df.index)
df2['Banking orders (2)'] = pd.Series(df['Banking orders (2)'], index=df.index)
df2['Target (Total orders)'] = pd.Series(df['Target (Total orders)'], index=df.index)
...
sns.jointplot(x='Week', y='Target (Total orders)', data=df, kind='reg', height=8);
plt.show();
...
sns.jointplot(x='Day', y='Target (Total orders)', data=df, kind='reg', height=8);
plt.show();
...
sns.jointplot(x='Non-urgent order', y='Target (Total orders)', data=df, kind='reg', height=8);
plt.show();
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=0)
lr_uo = LinearRegression()
lr_uo.fit(X_train, y_train)
y_train_pred = lr_uo.predict(X_train)
y_test_pred = lr_uo.predict(X_test)
...
sns.jointplot(x='Urgent order', y='Target (Total orders)', data=df, kind='reg', height=8);
plt.show();
...
sns.jointplot(x='Order type A', y='Target (Total orders)', data=df, kind='reg', height=8);
plt.show();
...
sns.jointplot(x='Order type B', y='Target (Total orders)', data=df, kind='reg', height=8);
plt.show();
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=0)
lr_ordc = LinearRegression()
lr_ordc.fit(X_train, y_train)
y_train_pred = lr_ordc.predict(X_train)
y_test_pred = lr_ordc.predict(X_test)
...
sns.jointplot(x='Order type C', y='Target (Total orders)', data=df, kind='reg', height=8);
plt.show();
...
lr_fis.fit(X_train, y_train)
y_train_pred = lr_fis.predict(X_train)
y_test_pred = lr_fis.predict(X_test)
...
sns.jointplot(x='Fiscal sector orders', y='Target (Total orders)', data=df, kind='reg', height=8);
plt.show();
...
y_train_pred = lr_tr.predict(X_train)
y_test_pred = lr_tr.predict(X_test)
...
sns.jointplot(x='Traffic controller sector orders', y='Target (Total orders)', data=df, kind='reg', height=8);
plt.show();
...
sns.jointplot(x='Banking orders (1)', y='Target (Total orders)', data=df, kind='reg', height=8);
plt.show();
...
sns.jointplot(x='Banking orders (2)', y='Target (Total orders)', data=df, kind='reg', height=8);
plt.show();
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=0)
lr_bank3 = LinearRegression()
lr_bank3.fit(X_train, y_train)
y_train_pred = lr_bank3.predict(X_train)
y_test_pred = lr_bank3.predict(X_test)
...
sns.jointplot(x='Banking orders (3)', y='Target (Total orders)', data=df, kind='reg', height=8);
plt.show();

non_urgent_order= df['Non-urgent order'].values.tolist()
df3 = pd.DataFrame(non_urgent_order,columns=['Non-urgent order'])
df3['Urgent order'] = pd.Series(df['Urgent order'], index=df.index)
df3['Order type B'] = pd.Series(df['Order type B'], index=df.index)
df3['Order type C'] = pd.Series(df['Order type C'], index=df.index)
df3['Banking orders (1)'] = pd.Series(df['Banking orders (1)'], index=df.index)
df3['Banking orders (2)'] = pd.Series(df['Banking orders (2)'], index=df.index)
df3['Target (Total orders)'] = pd.Series(df['Target (Total orders)'], index=df.index)
